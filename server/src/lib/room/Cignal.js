...
class Cignal extends EventEmitter {
  ...
  handleSocketConnection({ peerId, peerName, socket, role }) {
    ...
    // if peer creation failed, send an error and stop processing.
    if (!peer) {
      try {
        socket.send(
          JSON.stringify({
            type: "error",
            reason: "Failed to create peer on server",
          })
        );
      } catch (e) {
        logger.warn("Could not notify socket about peer creation failure:%o", e);
      }
      return;
    }
